...
VOICE_FOLDER = os.getenv(
    "VOICE_FOLDER", default=os.path.join(os.path.dirname(__file__), "voices")
)
SCREENSHOTS_FOLDER = os.getenv(
    "SCREENSHOTS_FOLDER", default=os.path.join(os.path.dirname(__file__), "screenshots")
)
...
def find_virtualenv(start_dir="."):
    for root, dirs, files in os.walk(start_dir):
        ...
        if "bin" in dirs and "activate" in os.listdir(os.path.join(root, "bin")):
            return root
        if "Scripts" in dirs and "activate" in os.listdir(
